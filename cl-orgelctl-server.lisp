...
(defun start-orgel-server (&key (send-to-pd t) (protocol :udp))
  (let ((host (or (cl-user::create-slynk-server) "127.0.0.1")))
    ...
    (start-lisp-server :host host :send-to-pd send-to-pd :protocol protocol)
    (incudine:rt-start)
    (sleep 2)
    (start-orgel-gui)
...
(defun papierrohrorgel-start (&key (send-to-pd t) (protocol :udp))
  (setf *package* (find-package :cl-orgelctl))
  (cl-orgelctl:start-orgel-server :send-to-pd send-to-pd :protocol protocol)
  (setf *package* (find-package :cl-orgelctl)))

(export 'papierrohrorgel-start 'cl-user)

(papierrohrorgel-start :send-to-pd cl-user::*send-to-pd-on-orgel-server-startup*
                       :protocol cl-user::*orgel-server-osc-protocol*)
